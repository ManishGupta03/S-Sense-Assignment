...
const Dashboard = () => {
  return (
    <main className="bg-white p-4 pl-[70px]">
        ...
        <div className="grid grid-cols-2 gap-4 w-5/12 sm:w-12/12">
          ...
          {cardData.map((card, index) => (
            <div key={index} className="shadow rounded p-4">
              ...
              <span className={card.changeDirection === "up" ? "text-green-500" : "text-red-500"}>{card.changeDirection === "up" ? "↑" : "↓"}{card.change}</span>
            </div>
          ))}
        </div>

        {/* Component */}
        <div className="w-6/12 sm:12/12"> <Component /> </div>
      </div>

      {/* Graphs */}
      <div className="flex gap-10 flex-wrap mt-2 w-full pt-10" >
        <div className="shadow rounded w-5/12 sm:12/12" style={{ height: "300px" }} ><Linegraph /> </div>
        <div className="shadow rounded w-6/12 sm:12/12" style={{ height: "300px" }} >
          <div style={{ width: "50%", margin: "auto" }}><DoughnutChart /> </div>
        </div>
      </div>
